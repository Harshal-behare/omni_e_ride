...
export default function Dealers() {
  const { dealers, loading, error, fetchDealers } = useDealers()
  ...
  if (error) {
    return (
      <>
        ...
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            <p className="text-red-600 mb-4">Error loading dealers: {error}</p>
            <Button onClick={fetchDealers}>Try Again</Button>
          </div>
        </div>
        <Footer />
      </>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {dealers.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-600 text-lg">No active dealers found. Please check back later.</p>
            </div>
          ) : (
            <div className="grid lg:grid-cols-2 gap-12 mb-16">
              ...
              <div className="space-y-6">
                {dealers.map((dealer) => (
                  <Card key={dealer.id} className="hover:shadow-xl transition-all duration-300">
                    <CardHeader>
                      <div className="flex justify-between items-start">
                        <CardTitle className="text-xl text-[#1F2937]">{dealer.name}</CardTitle>
                        <Badge
                          variant={dealer.status === "active" ? "default" : "secondary"}
                          className={dealer.status === "active" ? "bg-green-100 text-green-800" : ""}
                        >
                          {dealer.status}
                        </Badge>
                      </div>
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <a href={`tel:${dealer.phone}`} className="text-[#3CB043] font-medium hover:underline">
                          {dealer.phone}
                        </a>
                      </div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <a href={`mailto:${dealer.email}`} className="text-[#3CB043] font-medium hover:underline">
                          {dealer.email}
                        </a>
                      </div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <p className="text-sm text-gray-500">Manager: {dealer.manager_name}</p>
                      </div>

                      {dealer.territory && dealer.territory.length > 0 && (
                        <div className="pt-2">
                          <p className="text-sm text-gray-500 mb-2">Service Areas:</p>
                          <div className="flex flex-wrap gap-1">
                            {dealer.territory.map((area, index) => (
